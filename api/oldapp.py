...
from helpers import apology, login_required, date, time12h, time24h, tominutes, days_in_month, overdue
...
def temp():
    ...
    if temp:
        fixed = []
    incomplete_tasks = db.execute("SELECT * FROM tasks WHERE status = 'incomplete' AND user_id = ? ORDER BY due_date", session["user_id"])
    complete_tasks = db.execute("SELECT * FROM tasks WHERE (status = 'completed' AND due_date > ?) AND user_id = ? ORDER BY due_date", date, session["user_id"])
    ...
    return render_template("temp.html", temp=temp, weektypes=weektypes, events=events, incomplete_tasks=incomplete_tasks, complete_tasks=complete_tasks, fixed=fixed, chosen_weektype=chosen_weektype, date=date)
...
def settings_color():
    if request.method == "POST":
        ...
        if not request.form.get("new_color"):
            ...
            db.execute("UPDATE users SET color = ? WHERE id = ?", request.form.get("new_color"), session["user_id"])
